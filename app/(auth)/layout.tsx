...
const AuthLayout = ({
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-secondary flex items-center justify-center px-4">
      {/* Floating back button that does not affect layout height */}
      <div className="fixed left-4 top-4 z-50">
        <Link
          href="/"
          className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-accent-foreground hover:text-slate-900"
        >
      ...
      <div className="w-full max-w-6xl">
        <div className="flex items-center justify-between gap-16">
          ...
          <section className="flex-1 max-w-xl">
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-slate-900 dark:text-accent-foreground">
              {isSignUp ? `Sign up to ${config.appDetails.brand} and meet the ease` : 'Log in to track your finances efficiently.'}
            </h1>
            <p className="mt-6 text-base text-slate-500 dark:text-accent-foreground leading-relaxed">
